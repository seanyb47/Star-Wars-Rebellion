...
only part of the file Claude Code edits. Last updated 2026-09-14, 05:00: **scope locked by Sean — "generic speaking in several emotional states for both characters, that's all; voiceover later."** That is what ships: three mood faces per advisor and a speaking gesture sized to the mood (grave slow and small, encouraged quick and lifted) and to the character (the parrot bobs hard, the Secretary barely moves). Accent clips are cut from the code and the checker. Idle loops stay optional. Phase E (voice) is the next work. Before that, 2026-09-14, 04:45: **direction change — Rebellion's trick.** The droids in Star Wars: Rebellion never lip-sync; a message appears and the droid gestures until you have read it. So: the advisor now stands larger on the tab bar on every screen, wears the mood of whatever dispatch is on the card (face crops cut from the three stills), and rocks on their feet for as long as the line takes to read (`useAdvisorVoice`, code only, no video). The same gesture runs in the sheet's frame when an answer opens. **The twelve clips are now optional polish, not the plan**; any that exist still play. Before that, 2026-09-14, 04:20: **the narrator is in the game** — painted frame in the sheet, painted figures on the tab bar, idle clip when one exists, still otherwise; F1, F2, F4, F5, F6 done ahead of the sequencing rule because the visuals do not depend on the voice. Before that, 2026-09-14, 04:00: first clip in and exported as a seamless loop; clips paused on cost (300 credits each). Before that, 2026-09-14, late: Pennywhistle's three redone on a wooden quay (task 6) — filed, tab crop re-cut, checker 8/8; Drive stills folder verified ready for the clips task. Before that, 2026-09-14, night: Pennywhistle's background was the Imperium's capital — task 6 (Drive doc 6) redoes his three on a Confederacy quay; clips task re-issued with a frozen-background rule. Before that, 2026-09-14, later: **the six v2 stills are in** — bright, warm, painterly, Marlow in the Imperium's sea-green; filed, tab crops cut, checker 8/8. Phase C (clips) is open. Earlier that day: Marlow's costume changed from Admiralty black to the Imperium's Imperial-green and warm ivory (bible §16.1, Drive docs 1 and 5 re-issued); the redo has not been run yet. Earlier, 2026-09-13, night: **the first six stills were the wrong register** — dim, cold, near-photographic, against a game that is blue sky, warm light and appealing faces. They are retired to `stills/retired/v1-grim/` and all six are being redone against the game's own portraits (Drive `Narrators/style`), with the canonical style line in every prompt. Phase C waits on the redo.
...
| C1–C6 clips | HUMAN | **cut to optional idles** (1/6 made) | Speaking and mood are CSS gestures on the stills. Accents are gone. An idle loop per mood is optional polish; the one Marlow loop plays |
...
| F4 speaking gesture | CLAUDE CODE | done | replaces the accent timer: per-mood, per-character rocking for as long as the line takes to read |
