# Rebellion → Seven Seas, a translation table

Sean thinks in *Star Wars: Rebellion* (1998), because that is the game this one
is modelled on. He says so himself: *"I use a lot of SW Rebellion terms. So you
should note from a dev perspective which ones mean same thing."*

So this file is for me, not for the player. Two jobs:

1. **Understand him.** When he says "regiment" he means a Troop, and the
   right response is to do the Troop thing, not to ask which he means.
2. **Correct him, lightly.** He asked to be corrected when he uses a word the
   game no longer uses. One clause, in passing, and then get on with the work
   — *"(that's a Troop now)"* — never a lecture and never a blocked answer.

Nothing in this table is allowed anywhere a player can see it. The left column
is somebody else's trademarked game.

## The map

| Rebellion | Here | Note |
|---|---|---|
| Galaxy / galactic map | **World Map** | The tab you start on |
| Sector | **Reach** | A chain of 7–10 islands. The *Reach Map* is the screen |
| Sector map | **Reach Map** | |
| System / planet | **Location** (an island) | *Location* in a label, *island* in a sentence |
| Planet screen | **Location** | Harbor / Crew / Garrison / Buildings / Lore |
| Core / Rim | **Inner** / **Outer** Seas | A Sea names weather and paintings, nothing actable |
| Uncharted system | **Unexplored** | Charting it is the Explore errand |
| Hyperspace / jump | **Passage** / sailing | Days at sea, not a jump |

## People

| Rebellion | Here | Note |
|---|---|---|
| Personnel | **Crew** | Never "personnel" — retired 17 Sept |
| Character / officer | **Crew**, a *crew member* | Never the bare noun "officer" |
| Major character | **Pirate Lord** (Confederacy side) | The Crown's principals are just crew |
| Diplomacy (the rating) | **Parley** | |
| Diplomat (the role) | **Negotiator** | |
| Espionage / Combat / Leadership | unchanged | The other three ratings kept their names |
| Captured / imprisoned | **In irons**, a prisoner | Never released by the clock |
| Jedi / Force | **Deep-touched** / **the Deep** | See world bible §4 |

## Things you build

| Rebellion | Here | Note |
|---|---|---|
| Construction Yard | *(cut)* | No such building. A building is raised on the island itself, 20 September |
| Shipyard / Advanced Shipyard | **Shipyard** | One tier, and Craft grade does the upgrading |
| Training Facility | **Barracks** | Renamed 20 September; still where a Troop is raised |
| Mine | **Gold Mine** | Needs a vein under it |
| Refinery | **Lumber Mill** | Needs a forest |
| Planetary shield / Ion cannon | **Fortress** / **Heavy Fortress** | No landing while one stands |
| Facility (the category) | **Buildings** in the interface, **works** in prose | |
| Construction (the act) | **Building** | Nothing keeps the word now that the yard is cut |

## Forces

| Rebellion | Here | Note |
|---|---|---|
| Regiment / company | **Troop** | The garrison is the troops on an island. Company was the word from 17 to 19 September and is retired |
| Garrison | **Garrison** | Kept |
| Capital ship | **Hull**, a ship; large hulls are ships of the line | |
| Fighter squadron | — | Does not exist here |
| Fleet | **Fleet** / squadron | Kept |
| Death Star | — | Deliberately absent; the Leviathan is not a weapon |

## What you do

| Rebellion | Here | Note |
|---|---|---|
| Mission | **Errand** | ~15 days' work ashore after the sail |
| Diplomacy (mission) | **Parley** | |
| Incite Uprising | **Incitement** | |
| Espionage | **Espionage** | Kept |
| Sabotage | **Sabotage** | Kept |
| Recruitment | **Recruitment** | A Recruiter keeps an open table on loyal ground |
| Abduction / Rescue | **Abduction** / **Rescue** | Kept |
| Research / R&D | **Research**, raising **Craft** | |
| Assign to system/fleet | **Command** | |
| Bombardment | **Bombardment** | Kept |
| Invasion / assault | **Landing** | Military capture ≠ political allegiance |
| Loyalty / popular support | **Allegiance** | Out of 100, and the two sides' shares sum to it |
| Uprising | **Mutiny** | |

## Things Rebellion has that we do not

Fighters and squadrons, energy/mineral as two resources (we have gold only),
planetary shields as a separate layer from guns, the Death Star, droids,
sector-wide "influence" as a number the player reads.

## And one we have that it does not

The **watch** — how closely an island is being watched, which every covert
errand must get past before it is even attempted.
