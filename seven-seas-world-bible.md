...
- **2026-09-09 v2.8** — Sound, all synthesised in the browser so nothing is downloaded. Events have their own short sounds keyed to a new `kind` field on every event. The ambient bed is generated and never repeats. Each of the seven Seas has its own tuning, filter and reverb tail, so the Crown Sea sounds ordered, the Bone Sea sounds detuned against itself and the Glass Sea is nearly silent with an enormous tail; panning the chart retunes the bed. Islands in revolt darken it. Off by default, behind a speaker in the top bar.
