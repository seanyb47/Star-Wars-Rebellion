...
| Sector | **Reach** (an archipelago inside a Sea) | Same grouping as sectors. Seven Seas fixed; *number of Reaches per Sea* scales with map size. **On the small map there is one Reach per Sea, so a Sea and its archipelago are the same thing** (7 Reaches, 68 islands). Medium and large add second and third Reaches inside a Sea (→ ~150 / 200 islands). |
...
Sixty-eight islands rather than a hundred, and the chain view can lay every one
...
**Coral Reach** (orig. Sluis sector — `small` map; **13 islands charted**, across both Amber Sea archipelagos)
...
- **2026-09-13 v5.1** — **Coral Reach takes the second Amber Sea chain.** The painting has two archipelagos in the Amber Sea, and after v5.0 the long one down the right edge had no islands: Sugar Reach's had gone with its name, and Coral's label was landing on the empty chain. Coral now spans both — thirteen islands, within the fifteen the map allows — with Passh Bay, Denby Cay and Ortovale restored from its own list and Charmouth, Ferrol Key and Rishi Bank kept from Sugar's. Sixty-eight islands in all. Still seven Reaches; nothing else changes.
