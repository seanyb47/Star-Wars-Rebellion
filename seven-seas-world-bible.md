...
| **Admiral Dorian Jessup** | the ***Adamant*** — the Crown dreadnought he took with him when he left the Imperium's service. | The heaviest guns on the water, and every Confederate fleet lying in her harbor fights under the Admiral's command. |
...
(Capture targets for Faction A: **Calloway and Hale**, plus **the Open Deck**.)
...
| — | Alliance headquarters | **the *Open Deck*** | Unique (Lord) | Hale's ship, the Moot's deck. Never built, never replaced. See §2, Faction B. |
| — | Millennium Falcon | **the *Swallowtail*** | Unique (Lord) | Reyne's sloop. Fastest thing afloat; hit last. See §2, Faction B. |
| — | (none) | **the *Adamant*** | Unique (Lord) | Jessup's captured dreadnought. Heaviest guns; commands every Confederate fleet in her harbor. See §2, Faction B. |
...
- **2026-09-25 v9.74** — **Six islands a side, exactly.** Sean: *"I think there are too many starting locations. Let's scale it back… Total each side gets 6 only. Exactly."* It was **ten against eight or nine**, measured over eight seeds: the Crown three in Sovereign, two a side in each of three contested Reaches, Coralhome by name on top, and the Confederacy one or two in Sovereign plus Freeport. Now the Crown takes three in Sovereign, Coralhome and nothing else in Coral, and two split across Mire and Sunken; the Confederacy takes two in Sovereign, one in Coral, Freeport, and two split the same way. The outer pair is rolled **per side**, so where a side is thin changes war to war while how much it holds does not. **Measured, and the headline is a negative result:** 48 wars against the same seeds on the previous commit read **Crown 9 — Confederacy 39 both before and after** — not within noise, the identical number. Halving both openings does not touch the war's balance, so whatever is tilting it to four-fifths Confederate is somewhere else, and the three options written up earlier should be re-read knowing that. What the change does do is make the war smaller: both sides end far shorter of islands (Crown 18.2 → 12.4, Confederacy 15.9 → 8.6), the Confederacy's end-state fleet halves from 30 hulls to 15.8, and the unaligned world stays unaligned much longer. **Five tests failed and only one was about the opening** — four were fixtures silently coupled to which islands the deal handed out, and are now independent of it rather than retuned to it. The fifth was better than that: the deposit-density test read 0.42 against a 0.41 ceiling, and splitting the sample showed untouched ground sitting on **0.3980 against a `DEPOSIT_CHANCE` of 0.40** while dealt islands read 0.4810 — because the test counts a mill as evidence of a deposit and the opening seeds mills onto a side's holdings. The roll was never wrong; the sample had the opening's own gifts in it, which is why the answer moved every time the deal did. No other rules changed.

- **2026-09-25 v9.73** — **A crew member's sheet is a portrait, a name, four numbers and a way to the book.** Sean: *"We just need portrait name and stats. And link to encyclopedia. No need for a bunch of fluff than they can also read in the encyclopedia."* Five blocks came off, and every one was already in the crew entry word for word — the bio, the Lord's hull with its thumbnail and blurb, the role badges, the people, and the Lord powers, which the Rules page lists for all three. **This is the fourth time the same ruling has been made** — the odds table on 21 September, the RECRUIT explainer, *"This explanation text is good in glossary. But not here"* — so it is recorded as a rule rather than a preference: **a screen you act from carries what you are acting on; the reference carries the rest, one tap away.** One paragraph stayed and was cut to a line, because it is not lore but a consequence of the button under it: sending a posted officer anywhere ends the posting. **And the cut found a bug it was not looking for.** With the prose gone the portrait is the screen, and the Imperator's was the drawn cameo — fourteen of forty crew have no painting yet, which is known, but the fifteenth was **Halvard Corvane, who has one**. He was painted, shipped and registered as *Lord Regent* Halvard Corvane and is *Grand Admiral* now; art is found by `slugify(name)`, so the rename quietly dropped his portrait and his face crop on the floor. Exactly the disease of v9.72 one day later, in a third place. Both files renamed, the register updated, and `portraits.test.ts` now fails if a painting in the folder answers to nobody — the direction that matters, since most of the missing art is legitimately missing and only an orphan is a bug. No rules changed.

- **2026-09-24 v9.72** — **A Lord's ship has one name, and a test says so.** Sean: *"change the name of jessups ship / there is no firepower... we've addressed these 2 many times, so we keep getting stuck here."* Both were addressed, both kept coming back, and neither was a decision that failed to land — **both have been correct in the code since the day they were made.** What neither had was anything that fails when a *copy* goes stale. Measured this morning: three documents, three names for Hale's ship, and the one a player actually sees in **none** of them — the game says **Open Deck** and **Adamant**, `CANON.md` said *Harbor* (the id, not the name) and *Adamant*, and §2 and §6 here said ***Free Harbor*** and ***Ironback***. All four now say what the game says. **And this changelog is part of why.** At v8.7 and again at v9.53 it recorded that §2 and §6 *"are the record of what was designed, not of what shipped"* — nobody ruled that; a session deferred, wrote the deferral down as reasoning, and every session after read it as canon and deferred again. A changelog entry says what changed; an open question belongs in PLAN.md as an open question, because a deferral written down as a decision manufactures doctrine that outlives everyone who remembers it was a shrug. The **Firepower** half is the same disease in developer prose: the rule has been locked since 18 September and `navycombat.test.ts` already makes it impossible to reintroduce in code, but `navy.ts` still opened with *"combat reads four numbers — Firepower, Hull, Speed, hasLongGuns — none of which the v2.4 roster carries yet"*, at the top of a file, where anybody orienting themselves reads first — and said it a second time at the foot. `shipnames.test.ts` now pins both: a Lord's ship must be named the same in the game and in every document naming that Lord, a retired name may never sit on the same line as the Lord it left, and nothing in `src/sim` may describe an engine that reads a Firepower number. It caught a third stale bible reference and a second stale sentence on its first run, both of which had survived a fix by hand minutes earlier. No rules changed.

- **2026-09-24 v9.71** — **Eight troops stop being a pike-and-hat glyph and become a squad.** Sean's squad thumbnails, delivered 1122x1402 — 4:5 to four decimal places — and shipped at 512x640 with **no crop at all**. A new `troops` folder in the art register, so the masters are kept and a recrop is possible later; `art.py check` reports the same ten pre-existing ship problems and none from these. **The join needed a table, and finding out why was the whole job.** Art everywhere else in the game is found by slug — drop `blackfin.webp` in and the Blackfin has a painting — and that could not work here for two reasons, both of which would have shipped as a silently missing image: every delivered file carries a `-squad-v1` suffix that nothing strips, and two ids are not their filenames (Ship's Company is `crown-ships-company`, the Drowned Guard is `drowned-guard`). Renaming the files to match would have fixed both and thrown away the version, which is the one thing a second draft will need. So `troopart.ts` holds the mapping, and it earns its keep twice: it is also where a per-unit `object-position` lives. **One real display problem, found on the page.** The encyclopedia cell was `aspect-ratio: 4/3` with centred `contain` — right for a tall narrow glyph, and it letterboxed a 4:5 painting into big side gutters, drawing the squad *smaller* than the figure it replaced. The cell takes the art's shape now, and the figure cell was moved to 4:5 with it so a row holding both still lines up. The island's troop tile had the same shape problem in miniature: 51x64 inside a 165px tile, legible and nowhere near dominant; it goes through `art` rather than `icon` now and renders 131x163. **Nothing is cropped anywhere**, because every box the art is shown in is 4:5 — which matters, since the identifying half of each painting is in the top third (the Reefwalkers' ears, the Drowned Guard's plumes, the Tidewrought's helmets) and the bottom third is surf and spray. **Four units keep the drawn figure**: The Hushed and Bog Witches have drafts Sean is holding for revision and review, and Shoal Wardens and Urskin Berserkers have no squad art yet. The fallback was already the behaviour for all twelve and is untouched. No rules changed.

- **2026-09-24 v9.70** — **The island's first viewport answers the five questions, and the two orders come up to the surface.** Sean's island-command brief and prototype. Three of the five were already true of this sheet and were **two taps down**: what I have here, what is known ashore, and which orders I can give all lived inside a fleet card inside the Harbor tab under a heading called *Attack actions*, so a player looking at an enemy island had to open a ship to find out whether they could land on it. Now the panel opens with the painting, the allegiance bar, two force cards — **3 hulls · 94 guns / 3 of 6 troop berths filled** against **Ashore: Not counted** — and **Bombardment** and **Invasion** side by side and the same size, with the tabs and all their existing content underneath. The block is a second door onto the same rules and not a second set: the buttons call the same `onBombard` and `onAssault` the fleet card calls, read the same `bombardError` and `assaultError`, and print the same `bombardOdds`. **Two mechanics the prototype implied and the game does not have were left alone rather than invented**, as the brief asked: there is no stepper for a partial landing, because `assault` takes a fleet and no count and commits the whole hold; and there is no bombardment target picker, because Sean cut deliberate targeting on 21 September and shot goes at the walls while any stand. Each order opens a **read-only review** first — squadron, hulls, guns, rolls, what stands against them, and what it costs — because a landing is the one order in the game that cannot be called back and it used to be one tap with the numbers in a subtitle. **An uncounted garrison is not an empty one:** with no report the cards say *Not counted* and the invasion line reads *3 landing · nobody has counted what is ashore* rather than *3 against 0*. A shut order says why in ordinary type, not in muted tiny text. **And the painting scrolls now**, reversing the pinning of 20 September on the brief's explicit instruction — `flow`, the mode the encyclopedia already used, whose compact bar answers the objection the pinning existed for. No rules changed.

- **2026-09-24 v9.69** — **A passage is an errand, not a state.** Sean: *"Dont say at sea. Say 'Making for port, due in X days'."* The line was `At sea for Anchorite Rock — 6d`, which says where a squadron is and leaves the player to work out the only thing they came to the screen for. *At sea* is a condition; *due in six days* is a date you can plan a landing around. `fleetStatus` is the sim's one sentence for this and now reads **Making for Anchorite Rock, due in 6 days**, with his own wording — *Making for port, due in X days* — kept as the fallback for a landfall you cannot see, and *1 day* rather than *1 days*. The crew badge is too narrow for the sentence, so it carries **Making port 6d**, the shape the **Laid up 4d** badge beside it already uses, and it reads the days off whichever passage the person is on: their own errand, the leader's if they are only along for it, or the squadron's voyage. Elsewhere the phrase is a heading rather than a status and becomes **Under way** — the fleet list's first section, the chart's Fleets chip, the advisor's line about everybody being out — which is the word the Reach map's own legend already used. No rules changed.

- **2026-09-24 v9.68** — **A ring that is on every island says nothing.** Sean, with a fleet picked and the whole of Sovereign Reach ringed in green: *"I think the green rings are the issue. I don't mind the color change for neutral to yellowish but the rings make it hard to see anyone else. Do we need them?"* The prop's own comment gave the answer away — *"a fleet is choosing where to sail, and it can sail anywhere"* — so in sailing mode the pick ring was drawn **fifteen times out of fifteen**, a 52px green circle over every name and every loyalty bar, marking a set it was not narrowing at all. The other picking modes do narrow: placing a works lights only islands of yours, and a mission lights the ones offering it, colour-coded by the kind of work. So the condition is now the **set rather than the mode**, and a ring has two ways to earn its place: some islands are out of the running, **or** the ones in it mean different kinds of work — a brass ring for signing on and a broken brass one for covert work are not the green ring, and that difference is worth drawing even when everything is a target. The first cut asked only the first question and threw the second away, which would have been a real loss: measured in the browser on a day-40 save, picking a mission target in Sovereign Reach draws thirteen green and two broken brass, and the two are exactly the Confederate islands where the work would be covert. Both charts ask the same question, because a ring that appeared on zooming in and not on zooming out would be worse than either. Placing a works in that Reach now rings three islands and dims twelve; sailing rings none, and the sail cursor and the prompt carry the mode as they already did. No rules changed.

- **2026-09-24 v9.67** — **The Defenses tab answers two questions and stops.** Sean: *"Only things I care here are troops + if there is a commander. Don't need hulls or guns listed."* The tab was carrying a table where a count would do — a row for every battery standing, each with its own pair of figures, *6 against a landing · 4 against shot*, under a paragraph of siege rules. **Those figures are constants.** Every fort in the game carries the same pair and every heavy fort the same heavier pair, so the table printed the identical twelve lines on every island that has ever had a wall, and the one number that changes island to island — *how many are standing* — was the one thing it buried. That is a count, not a list, and the count is what is left, with the siege rules behind the ℹ on the Rules page where the rules live: the same move as the odds block off the character sheet on 21 September. **The commander is the addition**, and was not on this screen at all. It appeared only as a term inside the watch's arithmetic — *"76 from the chair"* — which answers how much harder a quiet mission against this island is, and does not answer *is somebody holding this place*, which is the question a player asks before landing on one. It reads off the island the panel is showing, so a remembered island names whoever had the chair **as of the report** rather than whoever has it now. No rules changed.

- **2026-09-24 v9.66** — **A mission an officer can do, and cannot do here today, says which.** Sean, day 40, the Imperator standing on Highwater with Recruit gone from the sheet: *"Why can't imperator recruit anymore by day 40?"* **Nothing was wrong.** Eight unaligned hands exist in a war, `RECRUITS_AT_START` = **two of them ashore on day one** and the other six drifting in across four hundred days, so the pool empties the moment those two are signed. Measured over six wars on each side: the pool first runs dry on **day 24 to 44 every time**, identically for both factions — because it is arrival and not the race that empties it — while the capital's allegiance sat at 90 against a threshold of 65 and never came near it. The rule is the design and is unchanged. What was wrong is that the sheet answered **by omission**, and an omission cannot be read: a card that is simply gone looks like a bug, and there was no way from that screen to learn otherwise. The game already had the explanation and put it in the wrong place — the "that is everybody currently ashore" notice fires in the **mission report** when the last recruit signs, twenty days before the question gets asked. So the two **role missions** — signing on and research, the two gated on who the officer is rather than on the island — now appear on the sheet greyed, marked *Not today*, with the reason where the gist would be: *"Nobody unclaimed is ashore anywhere. 6 more will come up out of the Seas as the war goes on."* Only for an officer who holds the role, so a cook is never told they cannot keep a table; the other eight stay omitted, because their absence is the island's own plain answer and nobody wonders why there is no rescue where there are no cells. `roleMissionBlock` walks the same conditions the offer walks, in the same order, so the sentence cannot name a cause that is not the one failing — including the two the sheet could never have guessed at, an empty world versus a drought, and unaligned hands ashore who will not sign *your* articles. No rules changed.

- **2026-09-24 v9.65** — **Press and hold opens a menu, and an island in mutiny flies a flag you cannot miss.** Sean: *"Maybe we need to implement press and hold as a feature… instead of click to always view encyclopedia maybe press and hold opens a menu"*, and *"We need a mutiny icon like the fleet icon… Something needs to indicate oh shit."* **The hold was already spoken for** — hold-to-drag is his own request of 19 September, rewritten on 20 September when the first try was *"super clunky"* — so the gesture splits the way a phone already does: hold and let go without moving opens the menu, hold and move picks the tile up. Nothing about the drag changes, and the menu costs a gesture nobody was using. A tile with one action gets no menu, because a menu of one is a second tap for nothing; the two that have menus are the two with two things to do, a troop (put aboard / encyclopedia) and a crew member (their sheet / encyclopedia). **The mutiny mark** was a small pennant in flat red with no outline, invisible over half the paintings in the game — it is twice the size now with the sails' three passes, a pale halo outside a dark keyline outside the fill, and it pulses, on both charts. No rules changed.

- **2026-09-24 v9.65** — **On the chart, only trouble moves.** Sean, with the World Map open on Idle crew: *"The constant flashing isn't clear what you're trying to bring to my attention."* The complaint is the plural. Three things were moving on that one screen, each meaning something different and none of them saying what: the **idle glow** breathing under Sovereign Reach, the **news ring** of the §20 propagation animation opening over Sunken Reach on a loop, and a **mutiny flag** pulsing beside it. Motion cannot carry three separate ideas at once — it stops being a signal and becomes weather, and then the one thing that genuinely wants to interrupt you cannot. So the rule is now one thing, and it is a rule about the chart and not about any one mark: **only trouble moves.** The idle glow holds still, which costs it nothing — it is a 52px disc on a chart whose largest dot is 18, so size and colour were always what found it. The news ring opens **once** per shock instead of filling all three days the rule keeps its note standing; a ring that opens out of the chain and fades is news arriving, and the same ring every 1.8 seconds for three days is a lighthouse. The mutiny flag keeps its pulse, alone, and means something again. **And the glow now says what it is.** A line of explanation under the chips was cut on 21 September as a caption on a picture that did not need one; it is back for three of the twelve, and only those three. The loud layers are the only ones that draw a mark no other layer draws, and a mark unique to one view is exactly the picture that does need its caption — the quiet nine still get none, because a star is a star on all of them and the chip above already named it. The sentence is the layer's own `hint`, the one the Almanac prints, so a filter cannot explain itself two ways. The chart's pills clear the strip by a **measured** height now rather than a written-down 66px, since the caption changes it. No rules changed.

- **2026-09-24 v9.64** — **Troops aboard by hand, beside the loading that does itself.** Sean, on Anchorite Rock — in revolt, blockaded, three troops ashore of six wanted: *"How do I move troops from an island into a fleet? I should be able to click on any troop and move any number of them into a fleet up to fleet troop limit."* The honest answer was that you could not, and that is his own ruling of 22 September coming back round — *"cut this ashore / aboard thing"* — after which the boats load themselves. **That stays**, because the reasoning holds for the common case: both ends of the old stepper only ever had one sensible answer. What it cannot do is the case he was looking at, because automatic loading takes only what an island can *spare* above its wanted garrison — so an island that is **short** will never give a troop up, however plainly those troops are about to be lost with it. So the control is back beside the automatic behaviour rather than instead of it. A troop tile is now the order and the ℹ corner mark takes the lookup, which is the tile component's own rule for a tile that has a job; it only becomes an order on ground you hold with a squadron of yours lying in the harbour. One row a squadron with a stepper capped by whichever runs out first, the quay or the holds, plus an **All N** because the common manual case is *take everything*. **It will strip an island bare and says so rather than stopping you** — under the wanted garrison the island stirs, and empty it falls to the next troop to arrive; both are real prices and sometimes worth paying, which is the reason to have the control at all. No rules changed.

- **2026-09-24 v9.63** — **A shipyard still going up is not a shipyard at work.** Sean, with Kingsward open: one shipyard standing idle with a Build ships button on it, a second going up with 66 days to run, and the build sheet refusing the order — *"Every shipyard of yours is at work."* One of them plainly was not. The trap is worth naming because it has caught three different pieces of code in one session: **a founding works carries a `building` order of its own**, and that order is the works *being built* rather than the works *building something*. `busyAt` found the half-built yard, `buildError` read *its* founding flag and answered "still being laid down" about a yard nobody had asked about, and the sheet folded that into "every shipyard is at work". So a works still going up no longer counts as busy — it does not exist yet, and it cannot stop the yard beside it taking an order — and the laid-down sentence is now only ever about the works you actually asked. **The halving was already the rule and already worked:** `daysToFinish` is `workLeft / crewOn` asked fresh every morning and `crewOn` counts only works that are standing, so the day a second yard opens the hull on the stocks takes two days of work a day. Nothing in the suite walked a yard from founding to standing with a hull under way, which is how the bug beside it survived; something does now. No rules changed.

- **2026-09-24 v9.62** — **A crew member out on a mission looks like one, and every mission has a colour.** Sean: *"Maybe we can do something to better display people on mission. Like shrink their portrait. Add a color border corresponding to the mission type (and color type of the mission from the log and thumbnail). And add on bottom 'On Mission: [Mission Type]'."* All three, working together: **the rim** is the colour of that kind of work, so a board of six answers *what is everyone doing* before a word is read; **the smaller portrait**, 68% and inset off the tile's top edge, makes room for the label and reads as *away* rather than standing here; and **the label** carries the truth, two lines as asked — ON MISSION over the kind of work, with the days on the second line rather than taking a third. One prop on the tile and not three, so it cannot wear the rim of one mission and the name of another. **There was no mission colour before:** the log gave every mission the same grey dot and the choice sheet the same frame, so the one thing a glance could have told you was the one thing nothing said. Ten tokens now, grouped by the kind of work — gold and amber talk, ember and rose make trouble, violets do it quietly, cool colours build or look — and all ten clear of the faction green and red, because a rim that read as a *side* rather than a *job* is worse than no rim. To make the log possible, `GameEvent` now carries `missionType`: the type was only ever in the sentence, and a word in a string is not something a stylesheet can read. The same colour rings the painting on the card you pick the mission from. No rules changed.

- **2026-09-24 v9.61** — **Somebody holding an island is posted, not free.** Sean, on a commander's sheet: *"When I assign a commander to an island it doesn't reduce the filter for available crew. He isn't available now bc he's on a mission commanding."* The screenshot carried its own proof — a green **Available** badge over a paragraph saying they were not available for anything else. The cause: command is stored on the **island** (`commanderId`) and every screen asking *is this one free* asked the **person**, `status === 'available'` and no mission, both of which a commander still answers yes to, because holding a place is not a mission and does not change a status. So the Idle crew filter counted them from the chair, the advisor listed them as free, and the party list would carry them off on somebody else's errand. The AI never had the bug — it keeps a posted set — so the rule existed and only the player's side of the glass did not know it. One question, one answer: `commandingAt` and `isFreeCrew`, called by the filter, the advisor, the badge and the party list, with a test that fails if any screen grows its own copy. The badge reads **In command**, neutral rather than good. The paragraph was wrong as well: sending a commander anywhere *does* end the posting, by design, so it now says they count as posted rather than free and that sending them ends it. No rules changed.

- **2026-09-24 v9.60** — **Open ground counts as idle.** Sean, with Highwater open — a shipyard laying down an Interceptor, two fortresses, four mills and six open plots: *"Also include in idle buildings anywhere that can have something constructed on it. See how this is idle bc it has available land and nothing being built."* The filter was answering a narrower question than its own name: an idle slipway is a works you are not using, six empty berths are an island you are not using, and both cost a fortnight where something could have gone up. An island now counts as idle when it is yours and quiet, has somewhere to put a building — an open berth **or an unworked deposit**, which earns nothing until something stands on it — and **is not already raising one**, since one island raises one building at a time and a filter that lights an island you can give no order to sends you there for nothing. It is the works lane alone: Highwater's shipyard was busy with a hull, and a hull takes no ground. The count is *things you could set going here this morning*, so an idle slipway beside open plots reads 2. **Measured, and worth a ruling:** over six seeds with both sides played, the filter now lights 76–100% of the islands you hold (87% at day 800), because open ground is not an exception in this game but the normal state of an island — the filter has gone from a nag to a background. Shipped as asked rather than quietly thresholded; the three ways to sharpen it are written up in PLAN.md. No rules changed.

- **2026-09-24 v9.59** — **A list of your own navy.** Sean, on the hollow sail going in: *"Even if it's not there it shouldn't be invisible to me."* The right complaint about a bigger hole than the one just filled — after that change a squadron could be reached by opening the island it lies at or the island it sails for, and **both start from already knowing which island that is.** Nothing in the game listed your fleets, so a squadron you had forgotten about was one you could not look up. **Your fleets** is one list of every squadron you have, **at sea first** because those are the ones nothing else on screen can show you: a fleet at anchor is a solid sail and a card in its harbor, a fleet on passage is a hollow sail above an island it has not reached. Each row carries the sim's own status sentence, so the list and the island sheet cannot describe one voyage two ways, and a row opens the island it is at or bound for rather than growing its own orders — two places to give one order is how they drift apart. The door is a chip at the foot of the chart beside the blockade alarm, reading *2 Fleets · 1 at sea*: the count is the door, and *at sea* is appended only when some are, because that is the half of the answer the chart cannot draw. No rules changed.

- **2026-09-24 v9.58** — **A squadron under way is on the chart, and you can open it.** Sean, with an island open that he had no report on: *"When my fleet is en route to an island — friendly neutral or foe — I should see the icon above the island and also be able to click on it and see it."* Two halves, both failing. The sail above an island drew only for a fleet with no voyage, so giving the order took the squadron off the chart for the whole fortnight it was sailing — the one time you most want to know where it is. A voyage of yours now puts a sail above its destination: **hollow**, in your colour, with the days to go under it, against the solid sail that means hulls lying in that water. Yours only, because an enemy squadron at sea is what espionage is for; not gated on whether you have ever been ashore there, because you know where you sent your own ships; and off under the None filter with everything else. The key gained the second state of the one glyph rather than a fourth row it has no space for, and both are now drawn from the chart's own path so the key cannot drift from what it explains. The other half was that an island with no tabs had nowhere to click through *to* — and there are two such screens, charted-but-never-visited and held-against-you-and-never-looked-at. Both now list your own hulls, live, and nothing of theirs. No rules changed.

- **2026-09-24 v9.57** — **Trade comes in every morning; the bill falls due once a fortnight.** Sean: *"I noticed gold doesn't go up daily. Can we make it so gold ticks up daily but upkeep is on the fortnight?"* Both halves used to settle together on day fourteen. This does not contradict his ruling of 20 September — *"otherwise people are going to be looking at it like a stock chart"* — because the stock chart was the **net**, a figure jittering either side of nothing; income alone climbs, at a rate you set by taking islands and raising works. So the smooth half moves daily and the lumpy half stays lumpy, the fourteen-day totals are unchanged, and on settlement day income is credited before the bill is drawn. **Measured, and it is not free:** 96 wars on seeds 8000 and 9000 against the same seeds settling both halves fortnightly — Crown 17—79 with the daily tick, 28—68 without. Eleven wins where one SD is 4.90, so 2.2 SD and real. The mechanism is that both sides now hold more gold on average, because trade arrives before the bill instead of netting against it, and being richer amplifies whoever converts gold to power best — which is the Confederacy, with cheap hulls it can keep laying down against first-rates that want a lump. **It amplifies the standing imbalance rather than creating one:** the war was 71% Confederate before and is 82% after, and that imbalance has been open since the fleet cut and is Sean's to rule on. Nothing has been tuned to hide it. No lore changed.

- **2026-09-24 v9.56** — **A build screen stops below the purse.** Sean: *"When on any build screen I need to see the gold section from the top bar so I can make purchase decisions."* The second half of his 22 September note, and the first half was not enough on its own: that pass raised the top bar over the sheet **while the purse is open**, which still works — but you have to tap the plaque to open the purse, and the plaque was behind the sheet. A sheet is 82% of the app measured up from the tab bar, which on a 393×852 phone lands its top edge at about 89px, above a bar that is 102px here and taller with a notch; the sheet asking for 675 gold was covering the only figure saying whether you had it. A sheet that spends or earns now caps its height by the bar instead (`--topbar-h`, measured by TopBar the way `--tabbar-h` has always been measured by TabBar, because only the device knows what its notch claims). A flag rather than a rule for every sheet: applying it everywhere would have shortened every sheet in the game to fix a screen about money, and most sheets are not that. The three that take it are the build menu, the build order sheet and the scrap sheet, which is the same decision backwards. The purse still expands over the sheet, so you see gold and the delta without doing anything and tap for Production and Upkeep. A side effect worth having: the foot of the sheet no longer runs off the screen, so **Where** — the island you are building at — is on screen instead of below the fold. No rules changed.

- **2026-09-24 v9.55** — **An island nobody holds reads as parchment, not as a third faction.** Sean, with a Reach map open and a mission half assigned: *"So hard to see blue from green on this screen."* Arithmetic rather than taste — the Crown's green and the old neutral light blue have almost exactly the same lightness, so the contrast between those two island names was **1.19:1**. They differed in nothing but hue, on the red-green axis, over a blue-green sea, on a phone held outdoors, for the one person in twelve with a colour deficiency; every one of those takes hue away and leaves nothing. An unheld island is now parchment (`--unheld`), which takes green against it to **1.70:1** and adds a warm/cool flip on top — the lightness gap is worth having and the flip is the part that survives an eye that cannot separate red from green at all. It is also truer to the fiction: the two sides own green and red, and an island under nobody's flag should read as the chart's own parchment rather than as a third side. `--neutral` keeps its blue for what is not an allegiance — an order in the log, a supernatural card. Two things fell out of it that were already wrong: **an uncharted island now recedes** (its name was 1.11:1 against parchment and is two stops dimmer now, still plainly legible on the water), and **the loyalty key lost a band that stood for nothing** — the swatch was a three-colour gradient while the bar it explains has had only two segments since the undecided middle was removed. A test now measures the palette rather than trusting it. No rules changed.

- **2026-09-24 v9.54** — **The music is on when you arrive.** Sean: *"Make game music play by default from start. Or at launch give people option. How do most games do this?"* Both, because on the web they are the same answer. Games ship with music on and a mute where you can find it; nobody opens with a dialog asking permission, which spends the player's first click on a question they cannot yet answer. The browser adds one rule that is not about preference at all — no note may sound until the player has pressed something — so the convention is to default the preference **on** and start the audio inside the first deliberate press, which every game has. Three changes. The stored preference is an **opt-out** (`!== 'off'`): it read `=== 'on'`, which made *never chose* and *chose silence* the same answer, and shipped a silent game with a commissioned score sitting in the bundle. **The theme starts on the side you press** rather than on Begin — the faction card is the first real press on that screen and the moment the game learns which of the two themes to lay down, so pressing the Crown brings up the Crown; continuing a save is the same press with the save's own side. And **the mute is on the title screen**, because the speaker lives in the top bar and there is no top bar yet. Checked in a browser with autoplay set to gesture-required: nothing before the first tap, the right side's theme on the tap, and no restart at Begin. Music, stingers and the advisor are still one switch; two sliders is the convention and is the next thing to do here. No lore changed.

- **2026-09-23 v9.53** — **The Ironback stops being Jessup's ship.** Sean: *"change ironback from jessups ship name to something else."* The hull was renamed at v8.7 — Jessup's dreadnought has been the ***Adamant*** since 18 September — but the *buildable* Ironback's encyclopedia entry was never rewritten, so it still opened *"taken out of Imperial service with Admiral Dorian Jessup still aboard"*. Two ships in the same book both claiming to be the one he left the Imperium with, one of them a class the Confederacy lays down in its own yards. The Ironback now has a legend of her own and no Admiral in it: a Crown two-decker taken at anchor in a fog and never given back, cut down, plated in bolted iron and re-armed with siege guns, her pattern laid down ever since — *the Crown builds the finest hulls on the water, and does not always keep them*. Changed in `COMBAT-MASTER-v4.3.md` Part 4 and parsed through to `ship-lore.json`, because the master is the source and `shiplore.test.ts` pins all 224 fields of it verbatim. The §2 and §6 tables still name the *Ironback* as Jessup's, as they still name Hale's the *Free Harbor*: unchanged at the v8.7 ruling that they record what was designed rather than what shipped. No rules changed.

- **2026-09-23 v9.52** — **Research becomes a trade, and two things that were built start being heard.** Sean: *"Only a small number of units should be able to research. Like 4 max in game."* Research was the one mission with no gate on it — recruiting has always wanted a **Recruiter** on the sheet and research took whoever was idle. It now wants **Drill Research** or **Ship Design**, which nine of the forty carry, and `openingCast` gives one of the drawn seats to somebody who can: a side opens with one in 96% of wars and two in the rest, with two more findable in the pool. That guarantee is the whole of the change — measured without it, **54% of sides opened with no researcher at all**, which would have removed a third of the game by shuffle, silently. Four is the ceiling rather than the lottery. Measured over 96 wars on seeds 8000 and 9000 against the same seeds ungated: Crown 28—68 with the gate, 25—70 without, three wins where one SD is 4.87. It does not move the war, which is the right answer for a rule about *who* does a thing: both sides lost the same freedom. **The advisor speaks.** *"I don't hear narrator voice."* — and every test was passing, because the honest state of that feature is silence until the voice settings here are filled in and `npm run voices` is run. Correct, and indistinguishable from broken. The browser now reads the written line instead, same manifest and same no-repeat rule, with the two advisors on different pitch and pace; a real recording always wins the moment one exists. **And seven kinds of news get seven instruments.** *"I don't hear unique chimes depending on type of log note."* Five of the seven were a plain sine ping at 587, 659, 784 or the bed's root — on a phone speaker over a drone, the same event four times, because pitch was doing all the work. Now a great bell for the war, a ship's bell for an island coming over, a drum roll and a sour note for a mutiny, guns for an action at sea, a signal lamp for a mission report, a hammer for a finished order, and one falling tone for a loss. A test fails the build if any two are built the same way; it caught mutiny and loss sharing a shape on its first run. The name of each row in the notification panel is now a button that plays it, because comparing two of them used to mean playing two wars. No lore changed.

- **2026-09-22 v9.51** — **The island's buildings become one list, and an island makes one thing at a time.** The Buildings tab was a board of 64px tiles with a stat block under each, then an "Order something built" section, then a catalogue of every works with its price — three places to look at one island, and the third was a list the build panel already is. It is now one row per thing: a 28px icon, its name, and *its own business* hard against the right edge. A maker shows the order it carries and the days left, or the button that opens the build panel; an earner shows what it brings in; a wall shows nothing, because the Defenses tab is where a wall's numbers live. Unworked ground and open plots are the same row with a **+** on it. Sean's words throughout: *"it should just have like shipyard and then the interface for what's going on in the shipyards next to it"*, *"that interface should be on the right side"*, *"change build hull here to just build ships"*, and on the sentence under the room bar, *"it says seven of 12 berths built. What the fuck's a berth?"* — the bar stays, the sentence explaining the bar does not. A whole developed island now fits one screen where it ran to a page and a half. **The fortress shows in two places on purpose**, at his ruling — named in Buildings because it occupies a plot, with its numbers in Defenses because that is the one screen answering what it would take to get the island off you. The rule change under it: **one island makes one thing at a time**, whatever is making it, so a shipyard and a barracks on the same island are no longer two separate jobs. Measured over 48 wars on seeds 3000, against the same seeds without it: the win split does not move at all (25—23 either way), wars run slightly shorter (median 717 against 780), and both sides finish with fewer hulls — the Confederacy 26.9 against 31.5, the Crown 21.6 against 22.9. It costs the side that concentrates yards on few islands more than the side that spreads, which is the Confederacy, and the war comes out even regardless. No lore changed.

- **2026-09-21 v9.50** — **The purse: gold, a delta, and the rest behind a tap.** Three of Sean's notes that were one idea — the economy shown in a unit nobody settles in, in more figures than anybody reads, under two different names. **Upkeep is a fortnight** everywhere it is shown, with no duration printed: *"you don't need to say how long. Just say 'Upkeep X' and gold symbol."* The books still run per day; per-day was simply the wrong unit to read, since the ledger settles fortnightly. **The top bar is one plaque** — what you have and which way it is going — and GOLD/IN/OUT/CLEAR becomes Available gold, Production, Upkeep and Surplus behind a tap. That retires rather than wins the old CSS fight over a 393px phone: four figures that no longer compete for the room need no rule to stop them shrinking. **Production** is the word for what an island earns, at Sean's ruling, over Income *and* over Earnings — a third word that was in his own sketch for this panel. No lore changed.

- **2026-09-21 v9.49** — **Missions get an encyclopedia page, and three sheets get shorter.** The odds table comes off the character sheet, the Recruit explainer off the island sheet, and the **Favorable** chip and its plus-and-minus factors off the mission cards — all three at Sean's word, and all three the same mistake: the *rules of missions* printed somewhere that looked like the state of one person or one island. They move into a new **Missions** page: how a mission works, then each one with its painting, what settles it and where it can be pointed. The one-line descriptions are shared with the sheet where you give the order rather than copied, and which rating settles which mission is read off the code rather than remembered. **Research** is the mission's name now, not "In the yards", and its grade makes *everything* quicker and cheaper — buildings, hulls and troops — where it used to touch hulls alone and so was worth nothing to a side not building ships that month. Measured at 8–12 over twenty wars against 9–11, which is inside the noise. No lore changed.

- **2026-09-21 v9.48** — **The Crown has to find Freeport again.** Sean, playing the Crown: *"Haha I just noticed something — if I play imperium it tells me where free port is lol."* Opening any unexplored frontier island put **Freeport** in the header, so the question the whole Crown campaign is built on could be answered by tapping round the map. Freeport is a real rename — the generator draws an uncharted island each war, renames it, and keeps the painted name in `chartName` — and that field already existed, but only for *position*: every view looked it up to decide where to draw the island, then printed the secret name as the label. **An island you have not explored now reads as the charts have it**, on the Location sheet, the Reach map, the Reach list and the mission target picker. You learn what a place is called by going there, which is the same rule as everything else about it. The drawn coastline keeps its old seed on purpose: it would otherwise change shape the moment the island was explored, and a pop is worse than the tell. Checked from both chairs on one seed — the Crown reads Varrow, the Confederacy reads Freeport — because a fix that hid it from everybody would have looked identical from the Crown's. No rules changed.

- **2026-09-21 v9.47** — **Saves survive a rename now.** Sean, on a day-33 game minutes after the Reach renames shipped: *"Whoa!!!! All island broke in wrong spots now."* Cinder, Whalers' and Wreckers' were floating in open water while Sovereign, Coral and Salt sat correctly — the three that broke being exactly the three that were renamed. A save carries its own Reach and island names; the chart ships with the build and is keyed `ReachName/IslandName`, so renaming either half makes every older save miss the lookup, and the fallback meant for an unpainted Reach rings the whole chain round the middle of the sea. No error, half the world wrong. `loadGame` maps the old names to the new ones now, `chartName` included, alongside the migrations it already carried for fleets, craft and the Boom. **The rule: a rename is a data edit and a save-format change at once, and only the first is visible while you write it** — anything renamed on the chart belongs in that table for as long as older saves might be opened. Tested by writing a save the way the old build wrote one and asserting every island resolves to painted ground. No rules changed.

- **2026-09-21 v9.46** — **Sixty-nine islands, and coral settled.** Sean: *"B - work with paint we have"*, and *"A3 is wrong. On the coral reach anytime a forest would have appeared instead make it coral at game start. That's it."* The coral half needed no work — that is exactly what the generator already does — so **A3 is cut**: no slipway-on-a-bed unlock, no Crown clearing, no placement setting. The six new islands go in on the painting as it stands: **Driftway** and **Nine Reefs** to Windward, **Singing Shallows** to Coral, **Keelhaven** to Rime, **Cane Hollow** to Mire, **Belfry Shoal** to Sunken. The detector already finds far more painted land than there are islands — nine spare in the tightest Reach and forty-one in the loosest — so each simply takes one more. Belfry Shoal drew the smallest rock on the chart, which is exactly right for one bell tower standing out of open water. The five cross-Reach moves stay dropped. Two bugs fell out of the changed dice: **the article helper was never applied to people**, so the recruitment dispatch had always read "put there by The Widow Ashgrave", and a **probabilistic rule was being asserted as a certainty** — the warm-island join test ran one world and one roll stream, and only ever held while its seed was lucky. Balance: 20 wars, Crown **9 — 11** Confederacy against 6 — 14 before the day's work, median 636, and Lords taken up from 0.90 to **1.45**. More islands means more places to hide, so the manhunt should be harder; it got easier, because the longer war gives the Crown time it never had. No rules changed.
